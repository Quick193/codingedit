...
4. Start both servers concurrently: `npm run dev` (backend on port 5000, frontend on port 5173). If port 5000 is taken, the backend will automatically try the next available port. The backend automatically loads environment variables from `backend/.env` even when started via root workspace scripts.
